...
test("request detail shows Not provided for empty rows and stacks its operational cards", async () => { const page = await source("../src/pages/ServiceRequestDetails.jsx"); const css = await source("../src/pages/ServiceRequestDetails.css"); assert.match(page, /empty \? "Not provided" : value/); assert.match(css, /@media \(max-width: 900px\)[\s\S]*grid-template-columns:\s*1fr/); });
