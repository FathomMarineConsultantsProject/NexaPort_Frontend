...
export default function InspectionWorkflowWorkspace(){
  ...
  return <main className="workflow-page workspace-page"><Link className="back-link" to="/admin/inspection-workflows"><ArrowLeft size={15}/> Inspection Workflow</Link><header className="workspace-head"><div><span className="workspace-reference">Request #{data.request.id}</span><h1>{data.request.reference}</h1><div className="workspace-meta"><span><Ship size={15}/>{data.request.vessel.name||"Vessel not provided"}</span><span><MapPin size={15}/>{data.request.port.name||"Port not provided"}</span><span>Required {date(data.request.requiredBy)}</span></div></div><div><span className="workflow-stage-chip">{lifecycleLabel}</span><Link to={`/requests/${data.request.id}`}>View Request <ExternalLink size={14}/></Link></div></header>{error&&<div className="workflow-alert error" role="alert">{error}</div>}<div className="workspace-layout"><WorkflowStageRail currentStage={data.workflow.currentStage} viewedStage={viewedStage} onView={setViewedStage} canViewNext={canViewNext}/><div className="workspace-stage">
