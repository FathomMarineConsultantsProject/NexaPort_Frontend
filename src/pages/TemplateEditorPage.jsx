...
export default function TemplateEditorPage() {
  ...
  const extract = async () => {
    ...
    try {
      ...
      if (controller.signal.aborted || extractionError.name === "AbortError") setError("Extraction cancelled.");
        ...
        const fallback = extension === "pdf" ? "Unable to read fields from this PDF. You can add fields manually." : "Unable to parse this XML file.";
        setError(isSafe ? raw : fallback);
      }
  ...
  const save = async () => {
    ...
    if (!fields.length) { setError("Add or extract at least one field before saving."); return; }
    if (!templateId && !sourceType) { setError("Extract a PDF, XML, DOCX or XLSX before saving this template."); return; }
    setBusy(true); setError(""); const layout = { extractionMethod: extractionMode || "manual", pageCount };
    try {
      if (!templateId) { const created = await createTemplate({ title, description, sourceType, extractionMethod: extractionMode, fields, layout }); setTemplateId(created.data.id); applyTemplate(created.data); navigate(`/templates/${created.data.id}`, { replace: true }); setMessage("Template fields saved. The original source file was discarded."); }
