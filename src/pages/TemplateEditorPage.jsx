...
export const fileError = (file, sourceMode) => {
  const source = SOURCE_CONFIG[sourceMode];
  if (!file) return source ? `Please select a ${source.label} file.` : "Choose a source file.";
...
export default function TemplateEditorPage() {
  ...
  const extract = async () => {
    ...
    try {
      const result = await extractTemplateSource(file, { sourceMode, signal: controller.signal, onProgress: ({ stage, percent }) => { setProgressText(stage); setProgress(percent); }, onWorker: (worker) => { ocrWorker.current = worker; } });
      const localFields = usefulLocalFields(result.fields); setFields(localFields); setLocalBaseline(localFields); setSourceType(sourceMode); setExtractionMode(result.mode); setPageCount(result.pageCount); setMappingEvidence(result.aiMappingRecommended ? { documentTitle: file.name.replace(/\.[^.]+$/, ""), sourceType: sourceMode, pagesOrSheets: result.evidence.pagesOrSheets } : null); setProgress(100); setMessage(result.message || `${localFields.length} local field suggestions are ready for review.`);
      ...
      if (controller.signal.aborted || extractionError.name === "AbortError") setError("Extraction cancelled.");
        ...
        const isSafe = safeMessages.some((msg) => raw === msg) || /^This document has \d+ pages; the supported (?:OCR )?limit is \d+\.$/.test(raw);
  ...
  const save = async () => {
    ...
    const validationErrors = localFieldErrors(fields); if (validationErrors.length) { setFieldErrors(validationErrors); setError(`${new Set(validationErrors.map((item) => item.index)).size} fields need attention before this template can be saved.`); return; }
