...
export default function FieldEditor({ fields, onChange, readOnly = false }) {
  ...
  return <section className="template-field-builder">
    ...
    {!fields.length ? <div className="template-empty compact"><strong>No fields configured</strong><span>Add a field manually or extract suggestions from the source file.</span></div> : fields.map((field, index) => <article className="template-field-row" key={field.fieldKey}>
      ...
      <div className="template-field-grid">
        <label>Label<input disabled={readOnly} value={field.label} onChange={(event) => update(index, { label: event.target.value })} /></label>
        <label>Type<select disabled={readOnly} value={field.type} onChange={(event) => update(index, { type: event.target.value, fieldType: event.target.value, options: event.target.value === "select" ? field.options : [] })}>{FIELD_TYPES.map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        <label>Section<input disabled={readOnly} value={field.section || ""} onChange={(event) => update(index, { section: event.target.value })} /></label>
        {field.type === "select" && <label className="template-field-wide">Options, separated by commas<input disabled={readOnly} value={(field.options || []).join(", ")} onChange={(event) => update(index, { options: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} /></label>}
        ...
        {field.type === "photo" && <PhotoFieldEditor field={field} onChange={(changes) => update(index, changes)} />}
