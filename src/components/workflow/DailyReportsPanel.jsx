...
function DailyReportEditor({requestId,report,onClose,onChanged}){
  const [form,setForm]=useState(()=>formFrom(report)),[mode,setMode]=useState(report.locked?"preview":"edit"),[busy,setBusy]=useState(false),[pendingAction,setPendingAction]=useState(""),[error,setError]=useState(""),[current,setCurrent]=useState(report);
  const [photo,setPhoto]=useState(null),[caption,setCaption]=useState(""),[area,setArea]=useState("");const fileRef=useRef(null),busyRef=useRef(false);
  const mutate=async(action,actionName="working")=>{if(busyRef.current)return null;busyRef.current=true;setBusy(true);setPendingAction(actionName);setError("");try{const response=await action();setCurrent(response.data);setForm(formFrom(response.data));await onChanged(response.data);return response.data;}catch(e){const fields=e.response?.data?.fieldErrors?.map((item)=>item.message).filter(Boolean).join(" ");setError([e.response?.data?.message||e.message,fields].filter(Boolean).join(" "));return null;}finally{busyRef.current=false;setBusy(false);setPendingAction("");}};
  ...
  const save=()=>mutate(()=>saveDailyReport(requestId,current.id,{reportDate:form.reportDate,data:data()}),"saving");
  const generate=async()=>{const result=await mutate(async()=>{if(!current.locked)await saveDailyReport(requestId,current.id,{reportDate:form.reportDate,data:data()});return generateDailyReportPdf(requestId,current.id);},"generating");if(result)setMode("preview");};
  const finalize=async()=>{const result=await mutate(async()=>{await saveDailyReport(requestId,current.id,{reportDate:form.reportDate,data:data()});return finalizeDailyReport(requestId,current.id);},"finalizing");if(result)setMode("preview");};
  const addPhoto=async()=>{if(!photo||busyRef.current)return;const result=await mutate(()=>uploadDailyReportPhoto(requestId,current.id,{file:photo,caption,inspectionArea:area}),"uploading");if(!result)return;setPhoto(null);setCaption("");setArea("");if(fileRef.current)fileRef.current.value="";};
  return <section className="daily-report-detail"><header><div><span>Inspection record / Day {current.dayNumber}</span><h3>Daily Inspection Report</h3><p>{dateLabel(current.reportDate)} · {current.status} · Prepared by {current.preparedBy?.name||empty}</p></div><div className="daily-detail-actions">{!current.locked&&<><button className={mode==="edit"?"active":""} onClick={()=>setMode("edit")}>Editor</button><button className={mode==="preview"?"active":""} onClick={()=>setMode("preview")}>Preview</button></>}<button aria-label="Close Daily Report" onClick={onClose}><X size={16}/></button></div></header>
    ...
    {pendingAction==="uploading"&&<div className="workflow-alert" role="status">Uploading photo...</div>}
    {mode==="preview"?<><DocumentPreview report={current}/><footer className="daily-report-actionbar"><span>{current.locked?`Finalized ${stamp(current.finalizedAt)}`:current.downloadUrl?"Generated PDF is ready. Finalize when the report is complete.":"Save, generate, then finalize when all required fields are complete."}</span><div>{current.downloadUrl&&<a className="workflow-secondary" href={current.downloadUrl} target="_blank" rel="noreferrer"><Download size={14}/> Download PDF</a>}{!current.locked&&<button className="workflow-secondary" disabled={busy} onClick={generate}>{pendingAction==="generating"?"Generating...":"Generate PDF"}</button>}{!current.locked&&<button className="workflow-primary" disabled={busy} onClick={finalize}>{pendingAction==="finalizing"?"Finalizing...":"Finalize Report"}</button>}</div></footer></>:<div className="daily-editor">
      ...
      <section><div className="daily-section-head"><div><h4>Photographic record</h4><p>Private evidence shown in report order. JPEG, PNG or WebP, up to 8 MB. Large camera photos are optimized before upload.</p></div></div>{current.photos?.length>0&&<div className="daily-photo-list">{current.photos.map((item,index)=><article key={item.id}><img src={item.previewUrl} alt={item.caption||`Photo ${index+1}`}/><div><strong>Photo {index+1}</strong><span>{item.inspectionArea||empty}</span><p>{item.caption||empty}</p></div><button aria-label={`Remove photo ${index+1}`} disabled={busy} onClick={()=>mutate(()=>removeDailyReportPhoto(requestId,current.id,item.id),"removing") }><Trash2 size={14}/></button></article>)}</div>}<div className="daily-photo-upload"><label><span>Photograph</span><input ref={fileRef} disabled={busy} type="file" accept="image/jpeg,image/png,image/webp" onChange={(e)=>setPhoto(e.target.files?.[0]||null)}/></label><label><span>Inspection area</span><input maxLength="120" value={area} onChange={(e)=>setArea(e.target.value)}/></label><label><span>Caption</span><input maxLength="240" value={caption} onChange={(e)=>setCaption(e.target.value)}/></label><button className="workflow-secondary" disabled={!photo||busy} onClick={addPhoto}><ImagePlus size={14}/> {pendingAction==="uploading"?"Uploading photo...":"Attach"}</button></div></section>
      <footer className="daily-report-actionbar"><span>Draft changes persist only after Save Draft.</span><div><button className="workflow-secondary" disabled={busy} onClick={save}><Save size={14}/> {pendingAction==="saving"?"Saving...":"Save Draft"}</button><button className="workflow-secondary" disabled={busy} onClick={generate}>{pendingAction==="generating"?"Generating...":"Generate PDF"}</button><button className="workflow-primary" disabled={busy} onClick={finalize}>{pendingAction==="finalizing"?"Finalizing...":"Finalize Report"}</button></div></footer>
    </div>}
  </section>;
}
